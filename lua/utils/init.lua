...
function M.tobool(value)
  ...
  elseif value == 1 then
    ...
    vim.notify("Attempting to convert data of type '" .. type(value) .. "' [other than 0 or 1] to boolean",
      vim.log.levels.ERROR, { title = "[utils] Runtime error" })
    return nil
  end
end
...
---在默认浏览器中打开当前光标下 url
function M.system_open_link(url)
  -- Open Link with OS handler (like gx)
  -- handlers
  local function win32_handler(url)
    -- implementation for Windows
    vim.fn.system('start "" "' .. url .. '"')
  end

  local function macunix_handler(url)
    -- implementation for macOS and Unix
    vim.fn.system('open "' .. url .. '" &')
  end

  local function linux_handler(url)
  ...
  local start_col, end_col = string.find(line, 'http[s]?://%S+', col)
  if start_col ~= nil and end_col ~= nil then
    url = string.sub(line, start_col, end_col)
  else
    return
  end
